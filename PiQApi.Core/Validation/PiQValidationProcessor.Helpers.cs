// PiQApi.Core/Validation/PiQValidationProcessor.Helpers.cs
...
public partial class PiQValidationProcessor
{
    ...
    private readonly ConcurrentDictionary<Type, Func<object, PiQValidationContext, Type, string, IPiQValidationResult>> _validateFuncCache = new();
    private readonly ConcurrentDictionary<Type, Func<object, PiQValidationContext, CancellationToken, Task<IPiQValidationResult>>> _validateAsyncFuncCache = new();

    /// <summary>
    /// Type-specific validation for any entity type
    /// </summary>
    private IPiQValidationResult ValidateTypedEntity<T>(T entity, PiQValidationContext context, Type entityType, string entityTypeName) where T : class
    {
        ...
        List<IPiQValidationRule<T>> typedRules = GetTypedRulesThreadSafe<T>(entityType);
        ...
        IPiQResourceScope? resourceScope = null;
        if (context.TryGetProperty("ResourceScope", out IPiQResourceScope scope))
        {
            resourceScope = scope;
        }
        ...
        foreach (var rule in typedRules)
        {
            try
            {
                ...
                var ruleResource = new PiQValidationRuleResource(
    ...
    /// </summary>
    private async Task<IPiQValidationResult> ValidateAsyncTyped<T>(T entity, PiQValidationContext context, CancellationToken cancellationToken)
        where T : class
    {
        ArgumentNullException.ThrowIfNull(entity);
        ArgumentNullException.ThrowIfNull(context);
        ...
        var validationResource = new PiQValidationRuleResource(
    ...
    /// </summary>
    private IPiQValidationResult ValidateWithTypeHandling(object entity, PiQValidationContext context, Type entityType, string entityTypeName)
    {
        ArgumentNullException.ThrowIfNull(entity);
        ArgumentNullException.ThrowIfNull(context);
        ...
        if (entityType == typeof(PiQValidationContext))
        {
            return ValidateTypedEntity((PiQValidationContext)entity, context, entityType, entityTypeName);
        }
        ...
        if (context.TryGetProperty("MetricsTracker", out IPiQOperationMetrics metrics))
        {
        ...
        try
        {
            ...
            if (context.TryGetProperty("MetricsTracker", out IPiQOperationMetrics completionMetrics))
            {
    ...
    /// </summary>
    private Func<object, PiQValidationContext, Type, string, IPiQValidationResult> CreateValidateFunc(Type entityType)
    {
        // Use expressions to build a compiled delegate (much faster than reflection)
        var entityParam = Expression.Parameter(typeof(object), "entity");
        var contextParam = Expression.Parameter(typeof(PiQValidationContext), "context");
        ...
        var method = typeof(PiQValidationProcessor).GetMethod(
        ...
        return Expression.Lambda<Func<object, PiQValidationContext, Type, string, IPiQValidationResult>>(
    ...
    /// </summary>
    private Func<object, PiQValidationContext, CancellationToken, Task<IPiQValidationResult>> CreateValidateAsyncFunc(Type entityType)
    {
        ...
        if (method == null)
        {
            ...
            return (entity, context, ct) => Task.FromResult<IPiQValidationResult>(
        ...
        // Compile the expression to a delegate
        return Expression.Lambda<Func<object, PiQValidationContext, CancellationToken, Task<IPiQValidationResult>>>(
    ...
    /// </summary>
    private List<IPiQValidationRule<T>> GetTypedRulesThreadSafe<T>(Type entityType) where T : class
    {
        if (!_rules.TryGetValue(entityType, out var rulesList))
        {
            return new List<IPiQValidationRule<T>>();
        }

        // Create a thread-safe copy
        lock (rulesList)
        {
            return rulesList.OfType<IPiQValidationRule<T>>().ToList();
        }
    }
    ...
    /// </summary>
    private async Task<IPiQValidationResult> ValidateEntityTypeAsync(object entity, PiQValidationContext context, Type entityType, CancellationToken cancellationToken)
    {
        ArgumentNullException.ThrowIfNull(entity);
        ArgumentNullException.ThrowIfNull(context);
