// PiQApi.Core/Validation/PiQValidationProcessor.cs
...
namespace PiQApi.Core.Validation
{
    ...
    /// </summary>
    public partial class PiQValidationProcessor : IPiQValidationProcessor
    {
        private readonly IServiceProvider _serviceProvider;
        private readonly IPiQValidationResultFactory _validationResultFactory;
        private readonly ILogger<PiQValidationProcessor> _logger;
        ...
        /// <summary>
        /// Initializes a new instance of the <see cref="PiQValidationProcessor"/> class
        /// </summary>
        ...
        public PiQValidationProcessor(
            IServiceProvider serviceProvider,
            IPiQValidationResultFactory validationResultFactory,
            ILogger<PiQValidationProcessor> logger)
        {
        ...
        public void RegisterRules<T>(IEnumerable<IPiQValidationRule<T>> rules) where T : class
        {
        ...
        public IPiQValidationResult Validate<T>(T entity, IPiQValidationContext context) where T : class
        {
        ...
        /// <returns>Validation result</returns>
        public Task<IPiQValidationResult> ValidateAsync<T>(T entity, IPiQValidationContext context, CancellationToken cancellationToken = default) where T : class
        {
            ArgumentNullException.ThrowIfNull(entity);
            ArgumentNullException.ThrowIfNull(context);
        ...
        /// <param name="context">Validation context</param>
        /// <returns>Validation result</returns>
        public IPiQValidationResult Validate(object entity, IPiQValidationContext context)
        {
            ArgumentNullException.ThrowIfNull(entity);
            ArgumentNullException.ThrowIfNull(context);
            ...
            var methodInfo = typeof(PiQValidationProcessor).GetMethod(
                nameof(ValidateInternal),
                System.Reflection.BindingFlags.NonPublic | System.Reflection.BindingFlags.Instance);

            var genericMethod = methodInfo?.MakeGenericMethod(entityType);
            return (IPiQValidationResult?)(genericMethod?.Invoke(this, new object[] { entity, context }))
        ...
        /// <param name="cancellationToken">Cancellation token</param>
        /// <returns>Validation result</returns>
        public async Task<IPiQValidationResult> ValidateAsync(object entity, IPiQValidationContext context, CancellationToken cancellationToken = default)
        {
            ArgumentNullException.ThrowIfNull(entity);
            ArgumentNullException.ThrowIfNull(context);
            ...
            {
                var task = (Task<IPiQValidationResult>?)genericMethod.Invoke(
        ...
        /// <param name="context">Validation context</param>
        /// <returns>Validation result</returns>
        private IPiQValidationResult ValidateInternal<T>(T entity, IPiQValidationContext context) where T : class
        {
            ...
            if (!_registeredRules.TryGetValue(typeof(T), out var rules) || rules.Count == 0)
            {
                ...
                var resolvedRules = _serviceProvider.GetServices<IPiQValidationRule<T>>().ToList();
            ...
            {
                if (rule is IPiQValidationRule<T> typedRule)
                {
        ...
        /// <param name="cancellationToken">Cancellation token</param>
        /// <returns>Validation result</returns>
        private async Task<IPiQValidationResult> ValidateInternalAsync<T>(
            T entity,
            ...
            CancellationToken cancellationToken) where T : class
        {
            ...
            var errors = new List<PiQValidationError>();
            var asyncRuleResults = new List<Task<IPiQValidationResult>>();
            ...
            foreach (var rule in rules)
            {
                ...
                else if (rule is IPiQValidationRule<T> syncRule)
                {
        ...
        /// <param name="context">Validation context</param>
        /// <returns>Validation result</returns>
        public IPiQValidationResult ValidateWithRules<T>(
            T entity,
            ...
            IPiQValidationContext context) where T : class
        {
            ...
            foreach (var ruleType in rulesList)
            {
                ...
                catch (Exception ex)
                {
                    ...
                    errors.Add(new PiQValidationError(
                        string.Empty,
