// PiQApi.Core/Validation/Diagnostics/PiQValidationDiagnosticsService.cs
...
namespace PiQApi.Core.Validation.Diagnostics;

/// <summary>
/// Service for validation diagnostics
/// </summary>
public class PiQValidationDiagnosticsService : IPiQValidationDiagnosticsService
{
    // Cache composite formats for frequently used format strings
    private static readonly CompositeFormat SummaryFormatComposite =
        System.Text.CompositeFormat.Parse(PiQValidationMessages.SummaryFormat);

    private static readonly CompositeFormat GroupHeaderFormatComposite =
        System.Text.CompositeFormat.Parse(PiQValidationMessages.GroupHeaderFormat);
    ...
    public string GetDiagnosticInfo(IPiQValidationResult result)
    {
    ...
    public string GetErrorReport(IPiQValidationResult result)
    {
        ArgumentNullException.ThrowIfNull(result);

        if (result.IsValid)
        {
            return PiQValidationMessages.ValidationSuccessful;
        }
        ...
        if (result.Errors.Count == 0)
        {
            return PiQValidationMessages.ValidationFailed;
        }
        ...
        foreach (var group in errorsByProperty)
        {
            ...
            var singleOrMultiple = errorCount == 1 ? PiQValidationMessages.SingleIssue : PiQValidationMessages.MultipleIssues;
            ...
            foreach (var error in group)
            {
                _ = sb.AppendLine(string.Format(CultureInfo.InvariantCulture,
                    "{0}{1}: {2}",
                    PiQValidationMessages.ListItemPrefix, error.Code, error.Message));
            }
