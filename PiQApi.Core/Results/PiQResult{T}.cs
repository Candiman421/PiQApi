// PiQApi.Core/Results/PiQResult{T}.cs
using PiQApi.Abstractions.Results;

namespace PiQApi.Core.Results;

/// <summary>
/// Generic implementation of IPiQResult
/// </summary>
/// <typeparam name="T">Result value type</typeparam>
public class PiQResult<T> : PiQResult, IPiQResult<T>
{
    /// <summary>
    ...
    protected internal static PiQResult<T> CreateSuccess(T value, string? correlationId = null)
    {
        ...
        return new PiQResult<T>(true, value, successMarker, correlationId ?? string.Empty, DateTimeOffset.UtcNow);
    }
    ...
    protected internal static new PiQResult<T> CreateFailure(string code, string message, string? correlationId = null)
    {
        var error = new PiQError(code, message, correlationId);
        return new PiQResult<T>(false, default, error, correlationId ?? error.CorrelationId, DateTimeOffset.UtcNow);
    }

    /// <summary>
    /// Initializes a new instance of the <see cref="PiQResult{T}"/> class
    /// </summary>
    ...
    public new IPiQResult<T> WithProperty(string key, object value)
    {
        ...
        return new PiQResult<T>(IsSuccess, Value, ErrorInfo, CorrelationId, Timestamp, newProperties);
    }
    ...
    public new IPiQResult<T> WithProperties(IDictionary<string, object> properties)
    {
