// PiQApi.Core/Core/PiQCorrelationContext.cs
...
/// </summary>
public sealed partial class PiQCorrelationContext : IPiQCorrelationContext
{
    // Using AsyncLocal ensures context flows across async boundaries
    internal static readonly AsyncLocal<PiQCorrelationContext?> _current = new AsyncLocal<PiQCorrelationContext?>();

    private readonly IPiQCorrelationIdFactory _correlationIdFactory;
    private readonly ILogger<PiQCorrelationContext> _logger;
    private readonly ConcurrentDictionary<string, object> _properties = new ConcurrentDictionary<string, object>();
    private readonly Stack<PiQCorrelationScopeState> _scopeStack = new Stack<PiQCorrelationScopeState>();
    private readonly PiQCorrelationContextOptions _options;
    ...
    private static readonly Action<ILogger, string, Exception?> LogCorrelationIdSet =
        LoggerMessage.Define<string>(
            ...
            "Set current correlation ID: {PiQCorrelationId}");
    ...
    private static readonly Action<ILogger, string, Exception?> LogInitialCorrelationCreated =
        LoggerMessage.Define<string>(
            ...
            "Created new correlation ID on first access: {PiQCorrelationId}");
    ...
    /// </summary>
    public static PiQCorrelationContext? Current => _current.Value;
    ...
    public static void SetCurrent(PiQCorrelationContext? context)
    {
    ...
    public PiQCorrelationContext(
        IPiQCorrelationIdFactory correlationIdFactory,
        ILogger<PiQCorrelationContext> logger,
        IOptions<PiQCorrelationContextOptions>? options = null)
    {
        _correlationIdFactory = correlationIdFactory ?? throw new ArgumentNullException(nameof(correlationIdFactory));
        _logger = logger ?? throw new ArgumentNullException(nameof(logger));
        _options = options?.Value ?? new PiQCorrelationContextOptions();
    }
    ...
    /// <param name="options">Configuration options</param>
    public PiQCorrelationContext(
        IPiQCorrelationId correlationId,
        IPiQCorrelationIdFactory correlationIdFactory,
        ILogger<PiQCorrelationContext> logger,
        IOptions<PiQCorrelationContextOptions>? options = null)
    {
    ...
    /// </summary>
    /// <returns>A concrete PiQCorrelationId instance</returns>
    /// <remarks>
    ...
    /// </remarks>
    private PiQCorrelationId GetOrCreateCorrelationId()
    {
        ...
        if (_current.Value == null)
        {
            ...
            return PiQCorrelationId.FromInterface(correlationId);
        }
        ...
        return PiQCorrelationId.FromInterface(_correlationIdFactory.CreateFromExisting(existingId));
    }
