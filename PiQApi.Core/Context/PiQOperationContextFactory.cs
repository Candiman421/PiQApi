// PiQApi.Core/Context/PiQOperationContextFactory.cs
...
/// </summary>
public class PiQOperationContextFactory : IPiQOperationContextFactory
{
    ...
    private readonly IPiQValidationResultFactory _validationResultFactory;
    ...
    public IPiQOperationContext Create(string operationName, IPiQCorrelationContext correlationContext)
    {
        ...
        var logger = _loggerFactory.CreateLogger<PiQOperationContextFactory>();
    ...
    /// <returns>Operation context</returns>
    public IPiQOperationContext CreateWithProperties(
    ...
    /// <returns>Operation context</returns>
    public IPiQOperationContext CreateConfigured(
        string operationName,
        IPiQCorrelationContext correlationContext,
        OperationType operationType,
        TimeSpan? timeout = null,
        IPiQOperationContext? parent = null,
        CancellationToken cancellationToken = default)
    {
        ...
        var logger = _loggerFactory.CreateLogger<PiQOperationContext>();

        // Create components for the operation context
        var identifier = new PiQOperationIdentifier(
            operationId,
            operationName,
            operationType,
            parent?.Identifier,
        ...
        var state = new PiQOperationState();
        var metrics = new PiQOperationMetrics(identifier.Id, identifier.Name);

        // Create the validation processor with the validation result factory
        var validationProcessor = new PiQValidationProcessor(
            _loggerFactory.CreateLogger<PiQValidationProcessor>(),
            _validationResultFactory);

        // Create the result factory for validation
        var resultFactory = new PiQResultFactory();

        // Create the validator
        var validator = new PiQOperationValidator(
            validationProcessor,
            resultFactory,
            _loggerFactory.CreateLogger<PiQOperationValidator>());

        // Create the resource context and resources
#pragma warning disable CA2000 // Suppresssed: ResourceManager and ResourceContext are owned by the PiQOperationContext
        // and will be disposed when the context is disposed through the resources property
        var resourceManager = new PiQResourceManager(_loggerFactory.CreateLogger<PiQResourceManager>());
        var resourceMetrics = new PiQResourceMetrics();
        var resourceContext = new PiQResourceContext(
            correlationContext,
            resourceMetrics,
            resourceManager,
            _exceptionFactory,
            _loggerFactory.CreateLogger<PiQResourceContext>());
#pragma warning restore CA2000

        var resources = new PiQOperationResources(
            resourceContext,
            _loggerFactory.CreateLogger<PiQOperationResources>());

        // Create the final operation context with all components
        var context = new PiQOperationContext(
            identifier,
            state,
            metrics,
            correlationContext,
    ...
    public IPiQOperationContext CreateChild(
        IPiQOperationContext parent,
    ...
    /// <param name="cancellationToken">Cancellation token</param>
    /// <returns>Operation context</returns>
    public IPiQOperationContext CreateStandalone(
        string operationName,
        ...
        CancellationToken cancellationToken = default)
    {
        ...
        var logger = _loggerFactory.CreateLogger<PiQCorrelationContext>();
        var correlationContext = new PiQCorrelationContext(correlationId, _correlationIdFactory, logger);
    ...
    /// <param name="cancellationToken">Cancellation token</param>
    /// <returns>Operation context</returns>
    public async Task<IPiQOperationContext> CreateInitializedContextAsync(CancellationToken cancellationToken = default)
    {
