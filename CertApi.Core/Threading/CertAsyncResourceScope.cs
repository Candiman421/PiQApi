// CertApi.Core/Threading/CertAsyncResourceScope.cs
...
/// </summary>
public sealed class CertAsyncResourceScope : IAsyncDisposable, ICertResourceScope, IDisposable
{
    private readonly List<IDisposable> _resources;
    private readonly CertResourceContext? _context;
    ...
    /// <summary>
    /// Initializes a new instance of the CertAsyncResourceScope class for direct resource management
    /// </summary>
    /// <param name="resources">The resources to include in the scope</param>
    /// <param name="logger">Optional logger instance</param>
    private CertAsyncResourceScope(IEnumerable<IDisposable> resources, ILogger? logger = null)
    {
    ...
    /// <summary>
    /// Initializes a new instance of the CertAsyncResourceScope class for resource context
    /// </summary>
    /// <param name="context">The resource context</param>
    /// <param name="resourceIds">The resource IDs to manage</param>
    /// <param name="logger">The logger</param>
    public CertAsyncResourceScope(CertResourceContext context, IEnumerable<string> resourceIds, ILogger logger)
    {
    ...
    public static async Task<CertAsyncResourceScope> CreateAsync(
        CertAsyncSemaphore semaphore,
        ILogger<CertAsyncResourceScope>? logger = null,
        CancellationToken cancellationToken = default)
    {
        ...
        return new CertAsyncResourceScope(new[] { releaser }, logger);
    }
    ...
    /// <param name="logger">Optional logger instance</param>
    /// <returns>A resource scope</returns>
    public static CertAsyncResourceScope Create(
        ...
        ILogger<CertAsyncResourceScope>? logger = null)
    {
        ...
        return new CertAsyncResourceScope(resources, logger);
    }
    ...
    public async Task<T> AddResourceAsync<T>(T resource, CancellationToken cancellationToken = default) where T : class
    {
        ...
        // If the resource context is available and resource implements ICertResource, track it
        if (_context != null && resource is ICertResource certResource)
        {
    ...
    public async Task RemoveResourceAsync<T>(T resource, CancellationToken cancellationToken = default) where T : class
    {
        ...
        // If the resource context is available and resource implements ICertResource, handle it there
        if (_context != null && resource is ICertResource certResource)
        {
