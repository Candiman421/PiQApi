// CertApi.Core/Core/CertResourceManager.cs
...
/// </summary>
public class CertResourceManager : ICertResourceManager
{
    ...
    private readonly ConcurrentDictionary<string, ICertAsyncLock> _resourceLocks = new();
    private readonly ILogger<CertResourceManager> _logger;
    private readonly ICertAsyncLockFactory _lockFactory;
    private readonly ICertCorrelationContext? _correlationContext;
    private readonly ICertAsyncLock _managerLock;
    ...
    /// <summary>
    /// Initializes a new instance of the <see cref="CertResourceManager"/> class
    /// </summary>
    ...
    public CertResourceManager(
        ILogger<CertResourceManager>? logger = null,
        ICertAsyncLockFactory? lockFactory = null,
        ICertCorrelationContext? correlationContext = null)
    {
        _logger = logger ?? Microsoft.Extensions.Logging.Abstractions.NullLogger<CertResourceManager>.Instance;
        _correlationContext = correlationContext;

        // If lock factory is not provided, create a new one
        _lockFactory = lockFactory ?? new CertAsyncLockFactory();
    ...
    public async Task<ICertResourceScope> CreateScopeAsync(string? scopeId = null, CancellationToken cancellationToken = default)
    {
        ThrowIfDisposed();
        ...
        var scope = new CertResourceScope(this, effectiveScopeId, _logger);
        return scope;
    }
    ...
    public async Task<T> TrackResourceAsync<T>(T resource, CancellationToken cancellationToken = default) where T : class
    {
        ...
        string resourceKey = resource is ICertResource certResource
...
#pragma warning restore CA2000
        ...
        try
        {
            ...
            foreach (var resourceId in resourceIdList)
            {
                ...
                else
                {
                    ...
                    throw new CertResourceLockedException(
                        resourceId,
        ...
        catch (CertResourceLockedException)
        {
