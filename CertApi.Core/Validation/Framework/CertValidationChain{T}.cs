// CertApi.Core/Validation/Framework/CertValidationChain{T}.cs
...
public class CertValidationChain<T> where T : class
{
    private readonly List<ICertValidationRule<T>> _rules = new();
    ...
    /// <summary>
    /// Initializes a new instance of the <see cref="CertValidationChain{T}"/> class
    /// </summary>
    ...
    public CertValidationChain<T> AddRule(ICertValidationRule<T> rule)
    {
    ...
    /// <returns>This chain for method chaining</returns>
    public CertValidationChain<T> AddRules(IEnumerable<ICertValidationRule<T>> rules)
    {
    ...
    /// </summary>
    /// <returns>This chain for method chaining</returns>
    public CertValidationChain<T> ClearRules()
    {
        _rules.Clear();
        return this;
    }
    ...
    public ICertValidationResult Validate(T entity, CertValidationContext context)
    {
        ...
        bool metricsAvailable = context.TryGetProperty("MetricsTracker", out ICertOperationMetrics metrics);
        if (metricsAvailable)
        {
        ...
        CertValidationResultBuilder builder = _resultFactory.CreateBuilder(correlationId);
        int errorCount = 0;
