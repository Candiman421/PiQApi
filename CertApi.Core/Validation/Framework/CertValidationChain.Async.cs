// CertApi.Core/Validation/Framework/CertValidationChain.Async.cs
...
public partial class CertValidationChain
{
    ...
    public async Task<ICertValidationResult> ValidateAsync<T>(T entity, CertValidationContext context,
        CancellationToken cancellationToken = default) where T : class
    {
        ...
        using (_asyncValidationChainScope(_logger, _chainName))
        {
            ...
            foreach (var processor in _processors)
            {
                ...
                result = (ICertValidationResult)_resultFactory.Combine(result, processorResult);
    ...
    /// </summary>
    private void StopMetrics(TimeSpan startTime, string metricKey, ICertOperationMetrics? metrics, bool metricsAvailable, int errorCount)
    {
        var elapsed = _timeProvider.GetElapsedTime(startTime);
        if (metricsAvailable && metrics != null)
        {
